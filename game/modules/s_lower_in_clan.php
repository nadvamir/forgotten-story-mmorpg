<?php
  ...
  if ($p['clan'][0] && $p['clan'][1] > 6)
  {
    ...
    if (isset ($_GET['to']))
    {
      ...
      if ($c['clan'][1] == 1) put_error ('вы неможете тут выгнать из клана');
      if ($c['clan'][1] == $p['clan'][1]) put_error ('нелзя понизить себя');
      // delaem:
      do_mysql ("UPDATE players SET clan = '".$c['clan'][0]."|".($c['clan'][1] - 1)."' WHERE id_player = '".$id."';");
      $f .= '<i>вы понизили игрока '.$to.' до клановой должности #'.($c['clan'][1] - 1).'</i><br/>';
    }
    ...
    while ($s = mysql_fetch_assoc ($q))
    {
      ...
      if ($s['clan'][1] == 1 || $p['clan'][1] == $s['clan'][1]) $f .= ($i + 1).'. '.$s['login'].' #'.$s['clan'][1].'<br/>';
      else $f .= ($i + 1).'. <a class="blue" href="game.php?sid='.$sid.'&action=lower_in_clan&to='.$s['login'].'&start='.$start.'">'.$s['login'].'</a> #'.$s['clan'][1].'<br/>';
      $i++;
    }
    ...
    for ($i = 0; $i <= $nw; $i++)
    {
      ...
      elseif ($i * $show < $c) $f .= '<a class="blue" href="game.php?sid='.$sid.'&action=lower_in_clan&start='.($i * $show).'">'.($i + 1).'</a> : ';
    }
    $f .= '<span class="gray">('.$c.')</span>';
    exit_msg ('понизить в клане:', $f);
  }
?>